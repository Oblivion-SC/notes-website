 ...
 * Исправлено: привязка аватара к пользователю
 */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ===== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ КЛЮЧЕЙ LOCALSTORAGE =====
    function getStorageKey(base) {
        return `${base}_${currentUserEmail}`;
    }
    function getAvatarKey() {
        return `profileAvatar_${currentUserEmail}`;
    }
    ...
    // ===== РЕНДЕРИНГ ЗАМЕТОК (с возможностью удалить ярлык) =====
    function renderNotes(filter = "") {
        ...
        const filtered = notes.filter(note => {
            ...
            if (searchText.includes(filter.toLowerCase())) return true;
            if (filter.startsWith("#")) {
                const tagQuery = filter.slice(1).toLowerCase().trim();
                if (note.labels && note.labels.length > 0) {
                    for (const labelId of note.labels) {
                        const label = labels.find(l => l.id == labelId);
                        if (label && label.name.toLowerCase() === tagQuery) return true;
                    }
                }
            }
            return false;
        ...
        filtered.forEach(note => {
            ...
            if (note.labels && note.labels.length) {
                labelsHtml = `<div class="note-labels">${note.labels.map(labelId => {
                    const label = labels.find(l => l.id == labelId);
                    return label ? `<span class="label-tag-wrapper">
    ...
    // ===== ПРОФИЛЬ И АВАТАР (с привязкой к пользователю) =====
    function loadAvatar() {
    ...
    // ===== ИЗМЕНЁННАЯ ФУНКЦИЯ РЕНДЕРИНГА ЗАМЕТОК ПО ЯРЛЫКУ =====
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                if (img.width > 300 || img.height > 300) {
                    ...
                    localStorage.setItem(getAvatarKey(), optimizedData);
                } else {
                    localStorage.setItem(getAvatarKey(), imageData);
                }
                loadAvatar();
    ...
    // ОБРАБОТЧИК КЛИКОВ ДЛЯ СЕТКИ ЗАМЕТОК
    notesGrid.addEventListener("click", (e) => {
        // Удаление ярлыка из заметки (крестик)
